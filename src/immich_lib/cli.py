...
def handle_check_auth(client, args):
    ...
    if info:
        print(f"Successfully connected to Immich! Server Info: {json.dumps(info, indent=2)}")
    else:
...
def handle_download_album(client, args):
    ...
    album_detail = client.get_album(album['id'])
    if not album_detail:
        print(f"Error: Could not retrieve details for album '{album.get('albumName', 'Unknown')}'.")
        return
    ...
    print(f"Downloading {len(assets)} assets from album '{album_detail['albumName']}'...")
...
def main():
    ...
    parser.add_argument("--url", help="Immich Server URL (defaults to IMMICH_SERVER_URL env var)")
    parser.add_argument("--key", help="Immich API Key (defaults to IMMICH_API_KEY env var)")
    
    subparsers = parser.add_subparsers(dest="command", help="Commands")

    # check-auth
    p_check_auth = subparsers.add_parser("check-auth", help="Verify connection and API key validity")
    p_check_auth.set_defaults(func=handle_check_auth)

    # list-albums
    p_list_albums = subparsers.add_parser("list-albums", help="List all accessible albums")
    p_list_albums.set_defaults(func=handle_list_albums)

    # list-assets
    p_list_assets = subparsers.add_parser("list-assets", help="List all accessible assets in the library")
    p_list_assets.set_defaults(func=handle_list_assets)

    # list-album-assets
    p_list_album_assets = subparsers.add_parser("list-album-assets", help="Show assets contained within a specific album")
    p_list_album_assets.add_argument("album_id_or_name", help="UUID or Name of the album")
    p_list_album_assets.set_defaults(func=handle_list_album_assets)

    # get-metadata
    p_get_metadata = subparsers.add_parser("get-metadata", help="Retrieve JSON metadata for a specific asset")
    p_get_metadata.add_argument("asset_id", help="UUID of the asset")
    p_get_metadata.set_defaults(func=handle_get_metadata)

    # download-album
    p_download_album = subparsers.add_parser("download-album", help="Download all assets from a specified album")
    ...
    if not url or not key:
        print("Error: Immich URL and API Key must be provided via environment variables or arguments.")
        print("Set IMMICH_SERVER_URL and IMMICH_API_KEY in your environment or a .env file.")
        sys.exit(1)
